...
{
  // INTCON
  GIE_GIEH = 1;  PEIE_GIEL = 1;  TMR0IE = 0;  INT0IE = 0;  RBIE = 0; // ãëîá.ïðåð/ãëîá.ïðåð âûñîê.óð;  ïðåð.ïåðèô./íèçê.óð.ïåðèô.ïðåð; òìð0.ïåðåï.ðàçð.ïðåð; âíåø.ïðåð.ðàçð; ïðåð.ïîðòÁ.ïðåð.
  // INTCON2
  RBPU = 0;  INTEDG0 = 0;  INTEDG1 = 0;  INTEDG2 = 0;  TMR0IP = 0;  RBIP = 0; // ïîðòÁ.ïóëàï(0-pullUp En; 1-pullUp Dis); ôðîíò.âíåø.ïðåð.0; ôðîíò.âíåø.ïðåð.1; ôðîíò.âíåø.ïðåð.2; òìð0ïðåð.ïðèîðèòåò; ïîðòÁïðåð.ïðèîðèòåò.
  // INTCON3
  INT2IP = 0;  INT1IP = 0;  INT2IE = 0;  INT1IE = 0; // ïðèîð.âíåø.ïðåð2; ïðèîð.âíåø.ïðåð1; ðàçð.âíåø.ïðåð2; ðàçð.âíåø.ïðåð1;
...
void ShutDownLB(void)
{
  LCD_erase();
  LATCbits.LC1 = 0;
  LCD_printstr8x5("Íèçêèé çàðÿä áàòàðåè!", 1, 0);
  LCD_printstr8x5("Óñòðîéñòâî", 3, 0);  
  LCD_printstr8x5("ñåé÷àñ âûêëþ÷èòñÿ!", 5, 0);
  LCD_erase();
  while(1);
}
...
void ShutDown(void)
{
  LCD_erase();
  LATCbits.LC1 = 0;
  LCD_printstr8x5("Âûêëþ÷åíèå...", 3, 0);
  LCD_erase();
  while(1);
}
